...
class Server(threading.Thread):
    ...
    def run(self, collect_metrics=True):
        ...
        while True:
            ...
            print("Connecting to", clipper_address)
            sys.stdout.flush()
...
class PredictionResponse:
    ...
    def add_output(self, output):
        ...
        output : string
        """
        if not isinstance(output, str) and not isinstance(output, bytearray):
            output = unicode(output, "utf-8").encode("utf-8")
        elif not isinstance(output, bytearray):
...
def add_metrics():
    config_file_path = '/container/metrics_config.yaml'
