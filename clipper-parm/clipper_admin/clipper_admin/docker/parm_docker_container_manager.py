...
class ParmDockerContainerManager(ContainerManager):
    def __init__(self,
        ...
        if not os.path.isdir(location):
            # Quick fix for running single-node experiments
    ...
    def start_clipper(self,
                      ...
                      frontend_args=None):
        """
        NOTE: This does not make use of the images passed in here and instead
        uses custom development images.
        """
        ...
        mode = frontend_args["mode"]
        my_volumes = {'{}{}'.format(self.location, mode) : {'bind': '/clipper/{}'.format(mode), 'mode': 'rw'}}
        print("MY_VOLUMES")
        print(my_volumes)
        ...
        if not self.external_redis:
            ...
            redis_container = self.docker_client.containers.run(
                ...
                name="redis-" + append_name,
                ports={'%s/tcp' % self.redis_port: self.redis_port},
                labels=self.common_labels.copy(),
                **self.extra_container_kwargs)
            self.redis_ip = redis_container.name
        self.redis_ip = "127.0.0.1"
        mgmt_cmd = ("/clipper/{mode}/src/management/management_frontend "
                "--redis_ip={redis_ip} --redis_port={redis_port}").format(
            mode=mode, redis_ip=self.redis_ip, redis_port=self.redis_port)
        mgmt_labels = self.common_labels.copy()
        mgmt_labels[CLIPPER_MGMT_FRONTEND_CONTAINER_LABEL] = ""

        self.docker_client.containers.run(
            "parm-query-frontend:latest", #mgmt_frontend_image,
            mgmt_cmd,
            name="mgmt_frontend-" + append_name,
            ports={
        ...
        query_cmd_str = ("/clipper/{mode}/src/frontends/query_frontend "
        ...
        if mode == "debug":
            query_cmd_str = ('bash -c "echo /clipper/debug/core.%h.%e.%t > /proc/sys/kernel/core_pattern && '
                            "ulimit -c unlimited && "
                            "/clipper/{mode}/src/frontends/query_frontend "
                            "--redis_ip={redis_ip} --redis_port={redis_port} "
                            "--prediction_cache_size={cache_size} "
                            "--redundancy_mode={redundancy_mode} "
                            "--num_models={num_models} "
        ...
        query_cmd = query_cmd_str.format(
                mode=mode,
                redis_ip=self.redis_ip,
                redis_port=self.redis_port,
                cache_size=cache_size,
                redundancy_mode=frontend_args["redundancy_mode"],
                num_models=frontend_args["num_models"],
                num_redundant_models=frontend_args["num_redundant_models"],
                batch_size=frontend_args["batch_size"],
                queue_mode=frontend_args["queue_mode"])

        print("QUERY CMD STR IS", query_cmd)

        query_labels = self.common_labels.copy()
        query_labels[CLIPPER_QUERY_FRONTEND_CONTAINER_LABEL] = ""
        query_name = "query_frontend-" + append_name
        self.docker_client.containers.run(
            "parm-query-frontend:latest",
            query_cmd,
            name=query_name,
            ports={
        ...
        # Metric Section
        query_frontend_metric_name = "query_frontend_exporter-" + append_name
        run_query_frontend_metric_image(
            query_frontend_metric_name, self.docker_client, "127.0.0.1",
    ...
    def deploy_model(self, name, version, input_type, image, num_replicas=1,
                     redundant=False, volumes=None, cmd=None):
        # Parameters
        # ----------
        # image : str
        ...
        self.set_num_replicas(name, version, input_type, image, num_replicas, redundant, volumes, cmd)

    def _get_replicas(self, name, version, redundant):
        containers = self.docker_client.containers.list(
            filters={
                "label":
                "{key}={val}".format(
                    key=CLIPPER_MODEL_CONTAINER_LABEL,
                    val=create_model_container_label(name, version, redundant))
            })
        return containers
    ...
    def _add_replica(self, name, version, input_type, image, redundant=False, volumes=None, cmd=None):
        ...
        model_container_label = create_model_container_label(name, version, redundant)
        labels = self.common_labels.copy()
    ...
    def set_num_replicas(self, name, version, input_type, image, num_replicas, redundant=False, volumes=None, cmd=None):
        current_replicas = self._get_replicas(name, version, redundant)
