...
class RequestHandler {
 public:
  RequestHandler(std::string address, int portno,
                 clipper::RedundancyType redundancy_mode,
                 clipper::QueueType queue_mode,
                 unsigned int num_models, unsigned int num_redundant_models,
                 unsigned int batch_size)
      : server_(address, portno),
        query_processor_(redundancy_mode, queue_mode, num_models,
    ...
    server_.config.thread_pool_size = 5;
    server_.add_endpoint(GET_METRICS, "GET",
                         [](std::shared_ptr<HttpServer::Response> response,
                            std::shared_ptr<HttpServer::Request> /*request*/) {
                           clipper::log_info(clipper::LOGGING_TAG_PARM, "Getting metrics");
    ...
    clipper::redis::subscribe_to_application_changes(
        ...
        [this](const std::string& key, const std::string& event_type) {
          ...
          if (event_type == "hset") {
            ...
            bool prefer_original = std::stoi(app_info["prefer_original"]) == 1;
            add_application(name, input_type, policy, default_output,
                            latency_slo_micros, prefer_original);
  ...
  void add_application(std::string name, InputType input_type,
                       ...
                       long latency_slo_micros, bool prefer_original) {
    ...
    auto predict_fn = [this, name, input_type, policy, latency_slo_micros,
                       prefer_original, app_metrics](
        std::shared_ptr<HttpServer::Response> response,
        std::shared_ptr<HttpServer::Request> request) {
      long cur_time_micros = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
      clipper::log_debug_formatted(clipper::LOGGING_TAG_PARM,
                                  "Got request at time={}",
                                  cur_time_micros
                                  );
  ...
  folly::Future<std::vector<folly::Try<Response>>> decode_and_handle_predict(
      ...
      long latency_slo_micros, bool prefer_original, InputType input_type) {
