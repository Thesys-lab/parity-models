  ...
  switch (type) {
    case MessageType::NewContainer: {
      ...
      if (!model_rpc_version) {
        ...
        log_info_formatted(LOGGING_TAG_PARM,
            "Adding container for model {}:{} with replica_id {}",
            model_name, model_version, cur_replica_id);
        ...
        connections_containers_map.emplace(
            connection_id,
            std::make_tuple(model, cur_replica_id,
                            connected_time));
    ...
    case MessageType::ContainerContent: {
      ...
      if (!new_connection) {
        ...
        TaskExecutionThreadPool::submit_job(
            vm, replica_id, container_ready_callback_,
            vm, replica_id);
      }
    } break;
