...
QueryProcessor::QueryProcessor(RedundancyType redundancy_mode, QueueType queue_mode,
                               unsigned int num_models, unsigned int num_redundant_models,
                               unsigned int batch_size)
  : state_db_(std::make_shared<StateDB>()),
  task_executor_(redundancy_mode, queue_mode, num_models,
                 num_redundant_models, batch_size) {
...
std::vector<folly::Future<folly::Unit>>
  create_wrapped_task_futures(
    vector<folly::Future<Output>>& task_futures,
    std::shared_ptr<std::mutex> outputs_mutex,
  ...
  return wrapped_task_futures;
}

folly::Future<Response> QueryProcessor::predict(
    Query query, bool prefer_original) {
  log_debug(LOGGING_TAG_PARM, "QueryProcessor::predict start");
  ...
  vector<folly::Future<Output>> task_futures;
  vector<folly::Future<Output>> backup_task_futures;
  std::tie(task_futures, backup_task_futures) =
  ...
  std::vector<folly::Future<folly::Unit>> wrapped_task_futures =
    create_wrapped_task_futures(task_futures, outputs_mutex, outputs_ptr);
   
  std::vector<Output> backup_outputs;
  backup_outputs.reserve(backup_task_futures.size());
  std::shared_ptr<std::vector<Output>> backup_outputs_ptr =
      std::make_shared<std::vector<Output>>(std::move(backup_outputs));

  std::vector<folly::Future<folly::Unit>> wrapped_backup_task_futures = 
    create_wrapped_task_futures(backup_task_futures, outputs_mutex, backup_outputs_ptr);

  folly::Future<folly::Unit> all_tasks_completed_future =
      folly::collect(wrapped_task_futures)
          .then([](std::vector<folly::Unit> /* outputs */) {});

  folly::Future<folly::Unit> all_backup_tasks_completed_future =
      folly::collect(wrapped_backup_task_futures)
          .then([](std::vector<folly::Unit> /* outputs */) {});

  std::vector<folly::Future<folly::Unit>> when_either_futures;
  when_either_futures.push_back(std::move(all_tasks_completed_future));
  if (!prefer_original) {
    // If the query has no preference over the original or backup tasks, then
    // we should only wait until the first of the two is complete.
    when_either_futures.push_back(std::move(all_backup_tasks_completed_future));
  }
  ...
  response_ready_future.then([
    outputs_ptr, backup_outputs_ptr, outputs_mutex, num_tasks, query, query_id,
    ...
    if (outputs_ptr->empty() && backup_outputs_ptr->empty() && 
        num_tasks > 0 && !default_explanation) {
      default_explanation =
    ...
    std::pair<Output, bool> final_output = current_policy->combine_predictions(
        selection_state, query, *outputs_ptr, *backup_outputs_ptr);
...
folly::Future<FeedbackAck> QueryProcessor::update(FeedbackQuery feedback) {
  ...
  vector<folly::Future<Output>> backup_predict_task_futures;
  std::tie(predict_task_futures, backup_predict_task_futures) =
