...
const std::pair<Output, bool> DefaultOutputSelectionPolicy::combine_predictions(
    ...
    std::vector<Output> predictions, std::vector<Output> backup_predictions) const {
  if (!predictions.empty()) {
    return std::make_pair(std::move(predictions.front()), false);
  } else if (!backup_predictions.empty()) {
    return std::make_pair(std::move(backup_predictions.front()), false);
  } else {
