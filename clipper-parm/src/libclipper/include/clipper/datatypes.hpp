...
class Query {
 public:
  ...
  // Whether the query should prefer the original outputs over those calculated
  // by a redundant unit.
  bool prefer_original_;
};
...
class Output {
 public:
  ...
  /*Output(Output&& other) {
    y_hat_ = other.y_hat_;
    models_used_ = std::move(other.models_used_);
  }*/
...
/*class SelectOutput {
 public:
  ...
  explicit SelectOutput() = default;
  SelectOutput(const SelectOutput &) = delete;
  SelectOutput &operator=(const SelectOutput &) = delete;

  SelectOutput(SelectOutput &&) = default;
  SelectOutput &operator=(SelectOutput &&) = default;

 private:
  folly::Promise<Output> original_;
  Output backup_;
};*/
...
class PredictTask {
 public:
  ...
  bool artificial_;
  size_t cache_id_;
  bool is_parity_ = false;
  std::chrono::time_point<std::chrono::system_clock> parity_time_;
};

class Batch {
 public:
  Batch() : batch_id_(0), redundant_(false) {}
  Batch(unsigned int batch_id, Deadline deadline) :
    batch_id_(batch_id),
    deadline_(deadline),
    redundant_(false) {}
  ...
  Batch(unsigned int batch_id, Deadline deadline,
    ...
    deadline_(deadline),
    redundant_(false) {}
  Batch(const Batch &other) = default;
