"""
This download script is adapted from
    ...
    with open(data_filename) as f:
        for line in f.readlines():
            ...
            if not os.path.exists(dest_fold):
                os.mkdir(dest_fold)
            shutil.move(os.path.join(original_fold, line[:-1]),
...
def create_train_fold(original_fold, data_fold, test_fold):
    ...
    for file in os.listdir(original_fold):
        if os.path.isdir(os.path.join(test_fold, file)) and file in dir_names:
            shutil.move(os.path.join(original_fold, file),
...
if __name__ == '__main__':
    ...
    parser.add_argument('download_directory', type=str,
            help='Path to the root folder of the downloaded dataset.')
    parser.add_argument('out_path', type=str,
            help='Path to which to save the split dataset.')
    args = parser.parse_args()
    make_dataset(args.download_directory, args.out_path)
